...
def fetch_tracks_by_date(session, headers, query, limit, offset):
    """Fetch tracks from Spotify by date range."""
    search_url = f"https://api.spotify.com/v1/search?q={query}&type=track&limit={limit}&offset={offset}"
...
def get_songs_data_by_release_date(
    start_date, end_date=None, limit=50, max_results=10000
):
    headers = {"Authorization": f"Bearer {get_token()}"}
    query = f"year:{start_date}" if not end_date else f"year:{start_date}-{end_date}"
    all_songs_data = []
    offset = 0
    ...
    while offset < max_results:
        try:
            tracks = fetch_tracks_by_date(session, headers, query, limit, offset)
            if not tracks:
                break
